...
export async function before(m, { conn }) {
  ...
  if (text.startsWith('.antinuke') || text.startsWith('.antiruba') || text.startsWith('.whitelist')) {
    ...
    return conn.reply(chatId, addFooter(` *COMANDI DISPONIBILI (solo owner)* .antinuke on / off / stato .antiruba on / off / stato .whitelist add @user .whitelist remove @user .whitelist list .whitelist reset `.trim()), m);
  }
