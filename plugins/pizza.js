let handler = async (m, { conn, args }) => {
  if (args[0] === 'pizza') {
    ...
    conn.sendMessage(m.chat, { text: 'Vuoi la tua pizza?', buttons, headerType: 1 });
  } else if (args[0] === 'pizza' && args[1]) {
    const condimento = args[1];
    const user = m.sender;
    const scelta = [condimento];
    conn.sendMessage(m.chat, { text: `Hai scelto ${condimento}. Vuoi aggiungere altro?`, buttons: [
    ...
    while (!sceltaCompleta) {
      ...
      if (risposta.text === 'si') {
        ...
        conn.sendMessage(m.chat, { text: 'Scegli un altro condimento', buttons, headerType: 1 });
        ...
        conn.sendMessage(m.chat, { text: `Questa è la pizza di @${user.split('@')[0]}: ${pizza}`, buttons: [
