    ...
    conn.ev.on('messages.update', (updates) => {
      for (const update of updates) {
        if (update.update.message?.editedMessage) {
          ...
          console.log(chalk.hex('#ffeb3b')('✏️ Messaggio editato:'), oldContent, chalk.hex('#00ff00')('->'), editedContent);
        } else if (update.update.message === null) {
          console.log(chalk.hex('#f44336')('🗑️ Messaggio eliminato:'), update.key.id);
        }
      }
    })
      ...
      try {
        ...
        if (!groupMeta) {
          ...
          if (groupMeta) {
            ...
            groupMetaCache.set(chatJid, groupMeta, { ttl: 300 });
          }
        }
...
function getColorScheme() {
  const custom = color => chalk.hex(color)
  return {
    label: custom('#00FF7F').bold,      // Spring Green
    text: custom('#FFFFFF'),           // Bianco
    secondary: custom('#9370DB'),      // Medium Purple
    meta: custom('#00CED1'),           // Dark Turquoise
    bright: custom('#FFD700'),         // Gold
    bold: chalk.bold,
    italic: chalk.italic,
    ...
    background: chalk.bgHex('#9370DB').white.bold,
    info: custom('#F8F8FF'),
    warning: custom('#FFA500'),        // Orange
    error: custom('#FF4500'),          // OrangeRed
    success: custom('#ADFF2F')         // GreenYellow
  }
}

function getBorders(c) {
  return {
    top: `${c.secondary.bold('╭──────────────────────────')}『 ${c.label.bold('BLOOD🩸BOT')} 』${c.secondary.bold('──────────────────────────╮')}`,
...
function logMessageSpecifics(m, c) {
  const specifics = {
    documentMessage: () => {
      ...
      console.log(`${c.secondary('📄')} ${c.text(fileName)}${c.meta(pages)} - ${c.gray(mimetype)}`)
    },
    contactMessage: () => {
      const name = m.msg?.displayName || 'Contatto'
      console.log(`${c.secondary('👤')} ${c.text(name)}`)
    },
    audioMessage: () => {
      const duration = formatDuration(m.msg?.seconds)
      const type = m.msg?.ptt ? 'Messaggio vocale' : 'Audio'
      console.log(`${c.secondary('🎵 ')} ${c.text(type)}${duration ? ' - ' + c.meta(duration) : ''}`)
    },
    videoMessage: () => {
      const duration = formatDuration(m.msg?.seconds)
      const gif = m.msg?.gifPlayback ? ' (GIF)' : ''
      console.log(`${c.secondary('🎥')} ${c.text('Video')}${duration ? ' - ' + c.meta(duration) : ''}${c.meta(gif)}`)
    },
    imageMessage: () => {
      const width = m.msg?.width
      const height = m.msg?.height
      const dimensions = width && height ? ` (${width}x${height})` : ''
      console.log(`${c.secondary('🖼️')} ${c.text('Immagine')}${c.meta(dimensions)}`)
    },
    stickerMessage: () => {
      const animated = m.msg?.isAnimated ? ' (Animato)' : ''
      console.log(`${c.secondary('🌟')} ${c.text('Sticker')}${c.meta(animated)}`)
    }
  }
...
async function formatText(m, conn) {
  ...
  if (m.isCommand) return chalk.bgHex('#00FF7F').black.bold(` ${text} `)
  if (m.quoted) return chalk.hex('#9370DB').italic(text.trim() || '[Messaggio Vuoto]')
  return chalk.whiteBright(text.trim() || '[Messaggio Vuoto]')
}

watchFile(__filename, () => {
   console.log(chalk.bgHex('#00FF7F').black.bold(" 🟢 File: 'lib/print.js' Aggiornato "))
})